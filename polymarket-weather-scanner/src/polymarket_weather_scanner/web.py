...
class ScannerWebHandler(SimpleHTTPRequestHandler):
    def __init__(self, *args, db: ScannerDatabase, web_dir: Path, **kwargs):
        self.db = db
    ...
    def handle_summary(self) -> None:
        rows = [dict(row) for row in self.db.latest_results(qualified_only=False, limit=1000)]
...
def serve(host: str = '127.0.0.1', port: int = 8765) -> None:
    ...
    def handler(*args, **kwargs):
        return ScannerWebHandler(*args, db=db, web_dir=WEB_DIR, **kwargs)
